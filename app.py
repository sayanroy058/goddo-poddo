...
CORS(app, supports_credentials=True, origins=["https://goddopoddo.com", "http://216.144.248.162"])
...
def auth_check():
    ...
    # Optionally load the user and check is_active/is_approved here
...
def view_draft_stories():
    ...
    # Bypass auth ONLY for Admin
    current_user = User.query.get(user_id)
    # if current_user and current_user.role == 'Admin':
    #     # Admin sees all draft stories
    #     drafts = Story.query.filter_by(STATUS='draft').all()
    if user_id:
        # Logged-in user sees only their draft stories
